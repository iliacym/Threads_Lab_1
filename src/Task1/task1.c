...
double TASK1_run(unsigned long long int const ntrials, int const nthreads) {
    pthread_t threads[nthreads];
    thread_data_t thread_data[nthreads];
    unsigned long long int const trials_per_thread = ntrials / nthreads;
    total_hits = 0;
    ...
    thread_data[nthreads - 1].trials = trials_per_thread + ntrials % nthreads;
    pthread_create(&threads[nthreads - 1], NULL, monte_carlo_pi, &thread_data[nthreads - 1]);
